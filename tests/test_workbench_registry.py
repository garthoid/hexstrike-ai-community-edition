"""
tests/test_workbench_registry.py

Pure-Python unit tests for server_core/workbench/registry.py — the
auto-discovery registry backing the Workbench data-transform toolset.
...
class TestParamSpecToDict:
    ...
    def test_includes_choices_when_present(self):
        spec = ParamSpec(name="algorithm", label="Algorithm", type="select", choices=["a", "b"])
        d = spec.to_dict()
        assert d["choices"] == ["a", "b"]

    def test_default_fields_present(self):
        spec = ParamSpec(name="x", label="X")
        d = spec.to_dict()
        assert d == {
            "name": "x",
            "label": "X",
