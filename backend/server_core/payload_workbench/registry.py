"""Operation registry for the Payload Workbench attack-payload toolset.

Every operation lives in its own module under server_core/payload_workbench/operations/<category>/
...
requires touching this file, server_api/payload_workbench, or the frontend.

Convention: every operation must have a param named "input" that carries its primary text —
this is what lets operations be chained into a Recipe (server_api/payload_workbench/routes.py's
...
def _discover() -> "dict[str, Operation]":
    ...
    import backend.server_core.payload_workbench.operations as _operations_pkg
    ...
    for _, category_name, is_pkg in pkgutil.iter_modules(_operations_pkg.__path__):
        ...
        for _, module_name, _ in pkgutil.iter_modules(category_pkg.__path__):
            module = importlib.import_module(
                f"backend.server_core.payload_workbench.operations.{category_name}.{module_name}"
            )
