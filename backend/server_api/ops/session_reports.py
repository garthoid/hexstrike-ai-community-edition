...
from backend.server_core.session_flow import load_session_any, append_event, verify_session_integrity
from backend.server_core.singletons import session_store, llm_client as _llm_client, run_history as _run_history
...
@api_session_reports_bp.route("/api/sessions/<session_id>/verify-integrity", methods=["GET"])
def verify_integrity(session_id: str):
  """Verify the tamper-evident hash chain of a session's run log."""
  try:
    ...
    logger.error(f"Error verifying integrity for {session_id}: {e}")
    return _bad(str(e), 500)
...
def _build_structured_report(
  ...
  lines.append(_integrity_summary_text(session_id))
  lines.append("")
...
<table style="border-collapse:collapse;width:100%;font-size:0.9em;">
  ...
  integrity_result = verify_session_integrity(session_id)
  if not integrity_result or integrity_result.get("total_runs", 0) == 0:
    integrity_html = '<p style="color:#6c757d;"><em>No run history recorded for this session.</em></p>'
  elif integrity_result.get("valid"):
    integrity_html = (
      f'<p style="color:#198754;">✅ {integrity_result["verified_runs"]}/{integrity_result["total_runs"]} '
      f'tool runs verified — evidence chain intact.</p>'
    )
  else:
    integrity_html = (
      f'<p style="color:#dc3545;">⚠️ Tamper detected — evidence chain breaks at run '
      f'#{integrity_result.get("broken_at_index")} '
      f'({integrity_result.get("verified_runs")}/{integrity_result.get("total_runs")} runs verified before the break).</p>'
    )
...
def _integrity_summary_text(session_id: str) -> str:
  """Human-readable evidence-integrity line for reports (markdown-safe)."""
  result = verify_session_integrity(session_id)
  if not result or result.get("total_runs", 0) == 0:
    return "*No run history recorded for this session.*"
  if result.get("valid"):
    return f"✅ {result['verified_runs']}/{result['total_runs']} tool runs verified — evidence chain intact."
  return (
    f"⚠️ Tamper detected — evidence chain breaks at run #{result.get('broken_at_index')} "
    f"({result.get('verified_runs')}/{result.get('total_runs')} runs verified before the break)."
  )
