...
def _discover() -> "dict[str, Operation]":
    ...
    import server_core.workbench.operations as _operations_pkg

    operations: "dict[str, Operation]" = {}
    for _, category_name, is_pkg in pkgutil.iter_modules(_operations_pkg.__path__):
        if not is_pkg:
            continue
        category_pkg = importlib.import_module(f"server_core.workbench.operations.{category_name}")
        for _, module_name, _ in pkgutil.iter_modules(category_pkg.__path__):
            module = importlib.import_module(
                f"server_core.workbench.operations.{category_name}.{module_name}"
            )
